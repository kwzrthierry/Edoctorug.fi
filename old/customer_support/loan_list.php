   ...
   <script>
        $(document).ready(function() {
            ...
            function filterTable() {
                ...
                input = document.getElementById("searchLoan");
                filter = input.value.toUpperCase();
                table = document.getElementById("loanTable");
                tr = table.getElementsByTagName("tr");

                // Get selected status filter value
                statusFilter = document.getElementById("filterStatus").value.toUpperCase();

                // Loop through all table rows, and hide those who don't match the search query and filter by status
                for (i = 0; i < tr.length; i++) {
                    tdName = tr[i].getElementsByTagName("td")[1]; // Index 1 corresponds to the 'Name' column
                    tdStatus = tr[i].getElementsByTagName("td")[7]; // Index 7 corresponds to the 'Status' column
                    if (tdName || tdStatus) {
                        txtValueName = tdName.textContent || tdName.innerText;
                        txtValueStatus = tdStatus.textContent || tdStatus.innerText;
                        if ((txtValueName.toUpperCase().indexOf(filter) > -1) && (statusFilter === "" || txtValueStatus.toUpperCase().indexOf(statusFilter) > -1)) {
                            tr[i].style.display = "";
                        } else {
            ...
            var loanId = $(this).data('id');
                // AJAX request to fetch user info
                $.ajax({
            ...
            $('.feedbackBtn').click(function() {
                var loanId = $(this).data('id');
                // AJAX request to fetch feedback
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#reasonText').html(response); // Reuse the same modal for displaying feedback
                        $('#reasonModalLabel').text('Feedback'); // Update modal title
                        $('#reasonModal').modal('show');
                    },
                    error: function(xhr, status, error) {
